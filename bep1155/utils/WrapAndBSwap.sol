 ...
 *         and pass their order to bswap. All funds will be returned
 ...
 */
contract WrapAndBSwap {

  IERC20Wrapper immutable public tokenWrapper; // ERC-20 to ERC-1155 token wrapper contract
  address immutable public exchange;           // BSwap exchange to use
  address immutable public erc20;              // ERC-20 used in bswap exchange
  address immutable public erc1155;            // ERC-1155 used in bswap exchange

  uint256 immutable internal wrappedTokenID; // ID of the wrapped token
  bool internal isInBSwap;               // Whether bswap is being called
   ...
   * @param _bswapOrder  Encoded BSwap order passed in data field of safeTransferFrom()
   */
  function wrapAndSwap(
    uint256 _maxAmount,
    address _recipient,
    bytes calldata _bswapOrder
  ) external
  {
    // Decode bswap order
    IBSwapExchange.BuyTokensObj memory obj;
    (, obj) = abi.decode(_bswapOrder, (bytes4, IBSwapExchange.BuyTokensObj));
    ...
    // Swap on BSwap
    isInBSwap = true;
    tokenWrapper.safeTransferFrom(address(this), exchange, wrappedTokenID, _maxAmount, _bswapOrder);
    isInBSwap = false;
    ...
    external returns(bytes4)
  {
    if (msg.sender != address(tokenWrapper)) {
      revert("WrapAndBSwap#onERC1155Received: INVALID_ERC1155_RECEIVED");
    }
    return IERC1155TokenReceiver.onERC1155Received.selector;
  }

  /**
   * @notice If receives tracked ERC-1155, it will send a sell order to bswap and unwrap received
  ...
  function onERC1155BatchReceived(
    ...
    external returns(bytes4)
  { 
    // If coming from bswap or wrapped token, ignore
    if (isInBSwap || msg.sender == address(tokenWrapper)){
      return IERC1155TokenReceiver.onERC1155BatchReceived.selector;
    } else if (msg.sender != erc1155) {
      revert("WrapAndBSwap#onERC1155BatchReceived: INVALID_ERC1155_RECEIVED");
    }

    // Decode transfer data
    IBSwapExchange.SellTokensObj memory obj;
    (,obj) = abi.decode(_bswapOrder, (bytes4, IBSwapExchange.SellTokensObj));

    require(
      obj.recipient == address(0x0) || obj.recipient == address(this), 
      "WrapAndBSwap#onERC1155BatchReceived: ORDER RECIPIENT MUST BE THIS CONTRACT"
    );

    // Swap on BSwap
    isInBSwap = true;
    IERC1155(msg.sender).safeBatchTransferFrom(address(this), exchange, _ids, _amounts, _bswapOrder);
    isInBSwap = false;
