...
import "../interfaces/IBSwapExchange.sol";
 ...
 */
contract BSwapExchange is ReentrancyGuard, ERC1155MintBurn, IBSwapExchange {
  ...
  constructor(address _tokenAddr, address _currencyAddr, uint256 _currencyID) public {
    require(
      ...
      "BSwapExchange#constructor:INVALID_INPUT"
    );
    factory = msg.sender;
  ...
  {
    // Input validation
    require(_deadline >= block.timestamp, "BSwapExchange#_currencyToToken: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountBought > 0, "BSwapExchange#_currencyToToken: NULL_TOKENS_BOUGHT");
   ...
   * @return price Amount of currency tokens to send to BSwap.
   */
    ...
    override public pure returns (uint256 price)
  {
    ...
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "BSwapExchange#getBuyPrice: EMPTY_RESERVE");
    ...
    internal nonReentrant() returns (uint256[] memory currencyBought)
  {
    ...
    // Input validation
    require(_deadline >= block.timestamp, "BSwapExchange#_tokenToCurrency: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(amountSold > 0, "BSwapExchange#_tokenToCurrency: NULL_TOKENS_SOLD");
    ...
    require(totalCurrency >= _minCurrency, "BSwapExchange#_tokenToCurrency: INSUFFICIENT_CURRENCY_AMOUNT");
   ...
   * @return price Amount of currency tokens to receive from BSwap.
   */
  ...
  {
    //Reserves must not be empty
    require(_assetSoldReserve > 0 && _assetBoughtReserve > 0, "BSwapExchange#getSellPrice: EMPTY_RESERVE");
    ...
    return numerator / denominator; //Rounding errors will favor BSwap, so nothing to do
  }
  ...
  {
    // Requirements
    require(_deadline >= block.timestamp, "BSwapExchange#_addLiquidity: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i ++) {
      ...
      require(_maxCurrency[i] > 0, "BSwapExchange#_addLiquidity: NULL_MAX_CURRENCY");
      require(amount > 0, "BSwapExchange#_addLiquidity: NULL_TOKENS_AMOUNT");

      // If the token contract and currency contract are the same, prevent the creation
      // of a currency pool.
      if (currencyPoolBanned) {
        require(tokenId != currencyID, "BSwapExchange#_addLiquidity: CURRENCY_POOL_FORBIDDEN");
      }
      ...
      if (totalLiquidity > 0) {
        ...
        */
        ...
        require(_maxCurrency[i] >= currencyAmount, "BSwapExchange#_addLiquidity: MAX_CURRENCY_AMOUNT_EXCEEDED");
        ...
        // See https://github.com/arcadeum/bswap/issues/19
        ...
        require(maxCurrency >= 1000000000, "BSwapExchange#_addLiquidity: INVALID_CURRENCY_AMOUNT");
  ...
  {
    // Input validation
    require(_deadline > block.timestamp, "BSwapExchange#_removeLiquidity: DEADLINE_EXCEEDED");
    ...
    for (uint256 i = 0; i < nTokens; i++) {
      ...
      require(totalLiquidity > 0, "BSwapExchange#_removeLiquidity: NULL_TOTAL_LIQUIDITY");
      ...
      require(currencyAmount >= _minCurrency[i], "BSwapExchange#_removeLiquidity: INSUFFICIENT_CURRENCY_AMOUNT");
      require(tokenAmount >= _minTokens[i], "BSwapExchange#_removeLiquidity: INSUFFICIENT_TOKENS");
    ...
    override public returns(bytes4)
  {
    ...
    // Users must be responsible and only use this BSwap exchange
    ...
    if (functionSignature == BUYTOKENS_SIG) {
      ...
      require(msg.sender == address(currency), "BSwapExchange#onERC1155BatchReceived: INVALID_CURRENCY_TRANSFERRED");
      require(_ids.length == 1, "BSwapExchange#onERC1155BatchReceived: INVALID_CURRENCY_IDS_AMOUNT");
      require(_ids[0] == currencyID, "BSwapExchange#onERC1155BatchReceived: INVALID_CURRENCY_ID");
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(token), "BSwapExchange#onERC1155BatchReceived: INVALID_TOKENS_TRANSFERRED");
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(token), "BSwapExchange#onERC1155BatchReceived: INVALID_TOKEN_TRANSFERRED");
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(this), "BSwapExchange#onERC1155BatchReceived: INVALID_NIFTY_TOKENS_TRANSFERRED");
    ...
    |__________________________________*/
      ...
      require(msg.sender == address(currency), "BSwapExchange#onERC1155BatchReceived: INVALID_TOKENS_DEPOSITED");
      require(_ids[0] == currencyID, "BSwapExchange#onERC1155BatchReceived: INVALID_CURRENCY_ID");

    } else {
      revert("BSwapExchange#onERC1155BatchReceived: INVALID_METHOD");
    }
    ...
    override public returns(bytes4)
  {
    ...
    require(
      ...
      "BSwapExchange#onERC1155Received: INVALID_ONRECEIVED_MESSAGE"
    );
   ...
   */
  fallback () external {
    revert("BSwapExchange:UNSUPPORTED_METHOD");
  }
    ...
    internal view returns (uint256[] memory)
  {
    ...
    if (nTokens == 1) {
      ...
      for (uint256 i = 1; i < nTokens; i++) {
        require(_tokenIds[i-1] < _tokenIds[i], "BSwapExchange#_getTokenReserves: UNSORTED_OR_DUPLICATE_TOKEN_IDS");
