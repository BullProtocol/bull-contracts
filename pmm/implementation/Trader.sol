...
import {IBSWAPCallee} from "../interface/IBSWAPCallee.sol";
...
contract Trader is Storage, Pricing, Settlement {
    ...
    function sellBaseToken(
        ...
        if (data.length > 0) {
            IBSWAPCallee(msg.sender).bswapCall(false, amount, receiveQuote, data);
        }
    ...
    function buyBaseToken(
        ...
        if (data.length > 0) {
            IBSWAPCallee(msg.sender).bswapCall(true, amount, payQuote, data);
        }
