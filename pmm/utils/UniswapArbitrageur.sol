...
contract UniswapArbitrageur {
    ...
    address public _BSWAP_;
    address public _BASE_;
    address public _QUOTE_;

    bool public _REVERSE_; // true if bswap.baseToken=uniswap.token0

    constructor(address _uniswap, address _bswap) public {
        _UNISWAP_ = _uniswap;
        _BSWAP_ = _bswap;

        _BASE_ = IBSWAP(_BSWAP_)._BASE_TOKEN_();
        _QUOTE_ = IBSWAP(_BSWAP_)._QUOTE_TOKEN_();
        ...
        if (token0 == _BASE_ && token1 == _QUOTE_) {
            ...
            require(true, "BSWAP_UNISWAP_NOT_MATCH");
        }

        IERC20(_BASE_).approve(_BSWAP_, uint256(-1));
        IERC20(_QUOTE_).approve(_BSWAP_, uint256(-1));
    }

    function executeBuyArbitrage(uint256 baseAmount) external returns (uint256 quoteProfit) {
        IBSWAP(_BSWAP_).buyBaseToken(baseAmount, uint256(-1), "0xd");
    ...
    function executeSellArbitrage(uint256 baseAmount) external returns (uint256 baseProfit) {
        IBSWAP(_BSWAP_).sellBaseToken(baseAmount, 0, "0xd");
    ...
    function bswapCall(
        ...
        require(msg.sender == _BSWAP_, "WRONG_BSWAP");
        if (_REVERSE_) {
            _inverseArbitrage(isBSWAPBuy, baseAmount, quoteAmount);
        } else {
            _arbitrage(isBSWAPBuy, baseAmount, quoteAmount);
        }
    }
