...
/**
 * @title BSWAPIncentive
 * @author BSWAP Breeder
 *
 * @notice Trade Incentive in BSWAP platform
 */
contract BSWAPIncentive is InitializableOwnable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // ============ Storage ============
    address public immutable _BSWAP_TOKEN_;
    ...
    event SetNewProxy(address bswapProxy);
    event SetPerReward(uint256 bswapPerBlock);
    ...
    function changePerReward(uint256 _bswapPerBlock) public onlyOwner {
        _updateTotalReward();
        bswapPerBlock = _bswapPerBlock;
        emit SetPerReward(bswapPerBlock);
    }
    ...
    function changeBSWAPProxy(address _bswapProxy) public onlyOwner {
        _BSWAP_PROXY_ = _bswapProxy;
        emit SetNewProxy(_BSWAP_PROXY_);
    }

    function emptyReward(address assetTo) public onlyOwner {
        uint256 balance = IERC20(_BSWAP_TOKEN_).balanceOf(address(this));
        IERC20(_BSWAP_TOKEN_).transfer(assetTo, balance);
    }
    ...
    function triggerIncentive(
        ...
        require(msg.sender == _BSWAP_PROXY_, "BSWAPIncentive:Access restricted");
        ...
        if (reward != 0) {
            IERC20(_BSWAP_TOKEN_).transfer(assetTo, reward);
    ...
    function _getTotalReward() internal view returns (uint256) {
        if (lastRewardBlock == 0) {
            ...
            return totalReward + (block.number - lastRewardBlock) * bswapPerBlock;
        }
    }
