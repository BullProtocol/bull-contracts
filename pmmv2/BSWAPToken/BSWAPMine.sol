...
contract BSWAPMine is Ownable {
    ...
    function getPendingReward(address _lpToken, address _user) external view returns (uint256) {
        ...
        return DecimalMath.mulFloor(user.amount, accBSWAPPerShare).sub(user.rewardDebt);
    }
    ...
    function getAllPendingReward(address _user) external view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            totalReward = totalReward.add(
                DecimalMath.mulFloor(user.amount, accBSWAPPerShare).sub(user.rewardDebt)
            );
        }
        return totalReward;
    }
    ...
    function deposit(address _lpToken, uint256 _amount) public {
        ...
        updatePool(pid);
        if (user.amount > 0) {
            uint256 pending = DecimalMath.mulFloor(user.amount, pool.accBSWAPPerShare).sub(
                user.rewardDebt
            );
        ...
        user.rewardDebt = DecimalMath.mulFloor(user.amount, pool.accBSWAPPerShare);
    ...
    function claimAll() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            updatePool(pid);
            pending = pending.add(
                DecimalMath.mulFloor(user.amount, pool.accBSWAPPerShare).sub(user.rewardDebt)
            );
            user.rewardDebt = DecimalMath.mulFloor(user.amount, pool.accBSWAPPerShare);
        }
        safeBSWAPTransfer(msg.sender, pending);
