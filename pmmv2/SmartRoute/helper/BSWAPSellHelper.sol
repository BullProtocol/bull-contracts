...
interface IBSWAPSellHelper {
    function querySellQuoteToken(address bswap, uint256 amount) external view returns (uint256);
    
    function querySellBaseToken(address bswap, uint256 amount) external view returns (uint256);
}
...
library BSWAPMath {
    ...
    /*
        Integrate bswap curve fron V1 to V2
        require V0>=V1>=V2>0
...
contract BSWAPSellHelper {
    ...
    function querySellBaseToken(address bswap, uint256 amount) public view returns (uint256) {
        return IBSWAPV1(bswap).querySellBaseToken(amount);
    }

    function querySellQuoteToken(address bswap, uint256 amount) public view returns (uint256) {
        BSWAPState memory state;
        (state.baseTarget, state.quoteTarget) = IBSWAPV1(bswap).getExpectedTarget();
        state.rStatus = RStatus(IBSWAPV1(bswap)._R_STATUS_());
        state.oraclePrice = IBSWAPV1(bswap).getOraclePrice();
        state.Q = IBSWAPV1(bswap)._QUOTE_BALANCE_();
        state.B = IBSWAPV1(bswap)._BASE_BALANCE_();
        state.K = IBSWAPV1(bswap)._K_();
        ...
        return
            DecimalMath.divFloor(
                boughtAmount,
                DecimalMath.ONE.add(IBSWAPV1(bswap)._MT_FEE_RATE_()).add(
                    IBSWAPV1(bswap)._LP_FEE_RATE_()
                )
            );
    }

    function _ROneSellQuoteToken(uint256 amount, BSWAPState memory state)
        internal
        pure
        returns (uint256 receiveBaseToken)
    {
        uint256 i = DecimalMath.divFloor(ONE, state.oraclePrice);
        uint256 B2 = BSWAPMath._SolveQuadraticFunctionForTrade(
            state.baseTarget,
            state.baseTarget,
    ...
    function _RAboveSellQuoteToken(uint256 amount, BSWAPState memory state)
        internal
        pure
    ...
    function _RBelowSellQuoteToken(uint256 amount, BSWAPState memory state)
        internal
        pure
        returns (uint256 receiveBaseToken)
    {
        ...
        uint256 i = DecimalMath.divFloor(ONE, state.oraclePrice);
        return BSWAPMath._GeneralIntegrate(state.quoteTarget, Q1, state.Q, i, state.K);
    }
}
